...
final class LoginViewController: UIViewController {
    ...
    @objc private func registerButtonTap() {
        ...
        Task {
            do {
                ...
                print("loginin=\(self.loginId),password\(self.password),nickname\(self.nickName)")
                
                let alert = UIAlertController(title: "계정생성 성공", message: "환영합니다,\(response.nickname)님! (ID:\(response.userId)", preferredStyle: .alert)
                ...
                let alert = UIAlertController(title: "계정생성 실패", message:error.localizedDescription,preferredStyle: .alert)
    ...
    @objc private func textFieldDidEditing(_ textField: UITextField) {
        ...
        case idTextField:
            ...
            print("loginID값\(loginId)")
        case passwordTextField:
            password = passwordTextField.text ?? ""
            print("password값\(password)")
        default:
            nickName = nickNameTextField.text ?? ""
            print("nickname값\(nickName)")
